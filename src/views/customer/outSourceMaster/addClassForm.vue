<template>
  <div class="addClassFormCon">
  <el-dialog  :title="edit?'编辑班务':'添加班务'" width="800px" :visible.sync="showAddClassForm" center :before-close="handleCancelAddClassForm">
    <el-form ref="form" :model="form" label-width="140px" :rules="formRules">   
      ...
      <el-col :span="22">
        ...
        <el-form-item label="选择发放方式:"  prop="isGrab" class="addRequireFixBug">
                    <el-radio-group v-model="form.isGrab" :disabled="edit"
                                    @change="isGrabChange">
                        <el-radio label="1">指派给绑定的班务经理</el-radio>
                        <el-radio label="2">发布至班务市场抢单</el-radio>
                    </el-radio-group>
                </el-form-item>
        
      <el-form-item label="班务经理:" prop="classManager" class="addRequireFixBug" v-show="form.isGrab == '1'" >
        <div v-if="!edit">
          <el-select v-model="form.classManager" placeholder="请选择" @change="handleChangeClassManager">
            <el-option v-for="(it,i) in managerList" :label="it.managerNameAlias" :key="i" :value="it.managerId"></el-option>
          </el-select>
        </div>
        <div v-else>{{nameForShow}}</div>
      </el-form-item>
      <el-form-item label="开始时间：">
              <div v-if="!edit">
              <el-col :span="12">
                <el-form-item prop="startDateTime">
                  <el-date-picker v-model="form.startDateTime"  type="date" @change="handleTest"  placeholder="选择日期" value-format="yyyy-MM-dd" format="yyyy-MM-dd"  :picker-options="pickerOptions0">
                  </el-date-picker>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item prop="startDateTime1">
                  <el-time-select v-model="form.startDateTime1"  placeholder="选择时间" value-format="HH:mm" format="HH:mm" @change="handleTest" :picker-options="{start: '00:00',step: '00:30',end: '23:30'}">
                  </el-time-select>
                </el-form-item>
              </el-col>
               </div>
            ...
            <el-form-item label="结束时间：" >
              <el-col :span="12">
                <el-form-item prop="endDateTime">
                  <el-date-picker v-model="form.endDateTime" type="date"  placeholder="选择日期" value-format="yyyy-MM-dd" format="yyyy-MM-dd" :picker-options="pickerOptions1">
                  </el-date-picker>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item prop="endDateTime1">
                  <el-time-select v-model="form.endDateTime1"  placeholder="选择时间" value-format="HH:mm" format="HH:mm" :picker-options="{start: '00:00',step: '00:30',end: '23:30'}">
                  </el-time-select>
                </el-form-item>
              </el-col>
            </el-form-item>
        <el-form-item label="结算方式:" required>
            <el-radio-group v-model="form.settlementType" :disabled="edit"  @change="settlementChange(form.settlementType)">
      ...
      <el-form-item label="客户核定人数:" prop ="customerNum">
        <el-col :span="22">
          ...
          <!--<div>用于统计生产分析报表 任务客户核定人数xxx人，最高可填写xx人</div>-->
          <div>用于统计生产分析报表</div>
        </el-col>
      ...
      <el-form-item label="零工单价审核" prop="needPriceCheck">
        <div>
                        <el-radio v-model="form.needPriceCheck" :label="2">不需要审核</el-radio>
                    </div>
                    <div>
                        <el-radio v-model="form.needPriceCheck" :label="1">全部排班审核</el-radio>
                        <span class="highlightText">班务下的排班都需审核通过后才能进行普工结算确认</span>
                    </div>
      </el-form-item>
      ...
      <div>
      <el-form-item label="已选中的工种:" class="addRequireFixBug" v-if="form.priceLimit===1" prop="alreadySelectWork">
        <el-table
          :data="form.workList"
          border
          style="width: 100%">
          ...
          <el-table-column
            ...
            >
            <template v-slot="scope">
                <el-input v-model="scope.row.hour" @change="(val)=>{handleDealListPrice(scope.row,scope.$index,'hour',val)}"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            prop="2"
            ...
            >
             <template v-slot="scope">
                <el-input v-model="scope.row.piece" @change="(val)=>{handleDealListPrice(scope.row,scope.$index,'piece',val)}"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            prop="3"
            ...
            >
             <template v-slot="scope">
                <el-input v-model="scope.row.bag" @change="(val)=>{handleDealListPrice(scope.row,scope.$index,'bag',val)}"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            prop="4"
            ...
            >
            <template v-slot="scope">
                <el-input v-model="scope.row.car" @change="(val)=>{handleDealListPrice(scope.row,scope.$index,'car',val)}"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            prop="5"
            ...
            >
            <template v-slot="scope">
                <el-input v-model="scope.row.ton" @change="(val)=>{handleDealListPrice(scope.row,scope.$index,'ton',val)}"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            prop="6"
            ...
            >
            <template v-slot="scope">
                <el-input v-model="scope.row.dayFixed" @change="(val)=>{handleDealListPrice(scope.row,scope.$index,'dayFixed',val)}"></el-input>
            </template>
          </el-table-column>
        </el-table>
      </el-form-item>
     ...
     <el-form-item label="日常产量确认人:" prop="confirm" >
        <el-checkbox-group v-model="form.confirm" @change="handleChangeConfirm">
          <el-checkbox v-for="(it,i) in allConfirmList" :key="i" :label="it.id" name="type">{{it.nameAlias}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
        <el-form-item label="保证金:" class="addRequireFixBug" v-show="this.form.isGrab == '2'" prop="depositProp">
            <el-col  :span="10">
...
-->
        ...
        <div v-if="priceForm.valuationType!==1">
          ...
          <el-form-item label="计量单价:" v-if="priceForm.calculateType===2">
             ...
             <el-col :span="20">
              <el-input v-model="priceForm.calculatePricePiece" @change="(val)=>{handleNumber(val,'calculatePricePiece','priceForm')}" ></el-input>
             </el-col>
              <el-col :span="4">
                元/件
               </el-col>
              ...
              <el-col :span="20">
                <el-input v-model="priceForm.calculatePriceBag" @change="(val)=>{handleNumber(val,'calculatePriceBag','priceForm')}" ></el-input>
              </el-col>
               <el-col :span="4">
                元/袋
               </el-col>
              ...
              <el-form-item prop="calculatePriceCar" style="margin-bottom:20px">
               <el-col :span="20">
                <el-input v-model="priceForm.calculatePriceCar"  @change="(val)=>{handleNumber(val,'calculatePriceCar','priceForm')}" ></el-input>
              </el-col>
               <el-col :span="4">
                元/车
               </el-col>
              ...
              <el-col :span="20">
                <el-input v-model="priceForm.calculatePriceTon" @change="(val)=>{handleNumber(val,'calculatePriceTon','priceForm')}" ></el-input>
              </el-col>
               <el-col :span="4">
                元/吨
               </el-col>
          ...
          <div v-if="priceForm.calculatePrice===1">
              <el-form-item prop="calculatePriceReal" v-if="priceForm.calculatePrice===1&&priceForm.calculateType===1">
                <el-col :span="20">
                  <el-input v-model="priceForm.calculatePriceReal" @change="(val)=>{handleNumber(val,'calculatePriceReal','priceForm')}" ></el-input>
                </el-col>
                <el-col :span="4">
          ...
          <div v-if="priceForm.calculatePrice===2&&priceForm.calculateType===1">
              ...
              <div>
                <i class="el-icon-delete" style="cursor:pointer" v-if="index == ladderList.length - 1" @click="handleDeleteLadder(index)"></i> 第{{index+1}}档:
                </div>
              ...
              <el-col :span="8">
                <el-form-item :prop="`ladder${index+1}price`">
                  <el-input v-model="priceForm[`ladder${index+1}price`]" @change="(val)=>{handleNumber(val,`ladder${index+1}price`,'priceForm')}" ></el-input>
                </el-form-item>
              </el-col>
            ...
            <el-form-item prop="exceedLadder">
              ...
              <el-col :span="20">
                <el-input v-model="priceForm.exceedLadder" @change="(val)=>{handleNumber(val,'exceedLadder','priceForm')}" ></el-input>
              </el-col>
               <el-col :span="4">
                元/{{unitMap[priceForm.calculateUnit]}}
               </el-col>
            ...
            <el-form-item>
              <div v-for="(item,index) in ladderList" :key="index">
                ...
                {{priceForm[`ladder${index+1}num`]?Number( priceForm[`ladder${index+1}num`] )+1:'X'}}
...
export default {
  ...
  data() {
      let checkIsGrab = (rule, value, callback) => {
          if( this.form.isGrab  ){
              callback();
          }else{
              callback(new Error("发放方式不能为空"));
          }
      };
      let checkdeposit = (rule, value, callback) => {          
          if( this.form.isGrab == '2' ){
              if( this.form.deposit ){
                  if( !integerValid.test(Number(this.form.deposit)) ){
            ...
            let checkClassManager = (rule, value, callback) => {
                if( this.form.isGrab == '1' ){
                    if( this.form.classManager ){
                        callback();
                    }else{
                        callback(new Error("班务经理不能为空"));
                    }
                }else{
                    callback();
                }
    ...
    unitMap:{
     ...
     formRules:{       
       ...
       classManager:[
           {validator: checkClassManager, trigger: 'change'},
       ],
        startDateTime:[{
        ...
        professionDemand:[{
         ...
         isGrab:[
             {validator: checkIsGrab, trigger: 'change'}
         ],
         depositProp:[
  ...
  watch: {
    showAddClassForm:function(val,old){ 
      if(val){         
        if(this.$store.state.outClassFormState){
          this.handleSetEditData(this.$store.state.outClassFormState);
        }
         this.handleInitForm();
      }else {
        this.$store.commit('outCLEAR_CLASS_FORM_STATE');
      }
    },
  ...
  methods: {
      isGrabChange(){
          this.updateForBug();
          this.$refs['form'].validateField('isGrab');
      },
      updateForBug(){
          this.$forceUpdate();
      },
    ...
    handleInitForm(){
    if(!this.edit){
      const {outClassListLen,outaskName,outThusTaskDetail} = this.$store.state;
      const {latitude,longitude,workingPlace,authorizedNumber,cusAuthorizedNumber,startTime,endTime,workStartTime,endWorkType,workEndTime,price,industryList,deposit,isGrab} = outThusTaskDetail;
     let $userInfo = JSON.parse(localStorage.getItem('userInfo'));
      this.form.fullName = $userInfo.customerName;
       this.form.classifyName = outaskName+'('+(outClassListLen+1)+')';
       ...
       this.form.customerNum=cusAuthorizedNumber;
        ...
        this.form.professionDemand = bridge;
       this.professionDemandForm={
       professionDemand:bridge
     };
    }
    },
    ...
    handleSetEditData(editData){
      ...
      this.nameForShow = editData.managerName;
    ...
    handleDealForm(val){
      ...
      if(valuationType===1){
        ...
        $price = {
          price:[{countTime:valuationPrice,ladderObj:[{count:"",unit:""}],countSettle:"",countChe:"",countDun:"",countDai:""}],unit:"",valuationType:1,settlementType:settlementType
        }
      }else if(valuationType===2&&calculateType===1&&calculatePrice===1){

         $price = {
          price:[{countTime:"",ladderObj:[{count:"",unit:""}],countSettle:unit===1?calculatePriceReal:"",countDai:unit===2?calculatePriceReal:"",countChe:unit===3?calculatePriceReal:"",countDun:unit===4?calculatePriceReal:""}],unit,valuationType:2,settlementType:settlementType
        }
      }else if(valuationType===2&&calculateType===1&&calculatePrice===2){

        $price = {
          price:[{countTime:"",ladderObj:[...priceList.map(it=>{
            return {
              count:it.num,
              unit:it.price
            }
          }),{overUnit:exceedPrice.price}],countSettle:"",countChe:"",countDun:"",countDai:""}],unit,valuationType:3,settlementType:settlementType
        }
      }else if(valuationType===2&&calculateType===2){

    $price = {
      price:[{countTime:"",ladderObj:[{count:"",unit:""}],countSettle:calculatePricePiece,countDai:calculatePriceBag,countChe:calculatePriceCar,countDun:calculatePriceTon}],unit:"",valuationType:4,settlementType:settlementType
    }
  }else if(valuationType===3&&calculateType===1&&calculatePrice===1){

    $price = {
      price:[{countTime:valuationPrice,ladderObj:[{count:"",unit:""}],countSettle:unit===1?calculatePriceReal:"",countDai:unit===2?calculatePriceReal:"",countChe:unit===3?calculatePriceReal:"",countDun:unit===4?calculatePriceReal:""}],unit,valuationType:5,settlementType:settlementType
    }
  }else if(valuationType===3&&calculateType===1&&calculatePrice===2){

    $price = {
      price:[{countTime:valuationPrice,ladderObj:[...priceList.map(it=>{
            return {
              count:it.num,
              unit:it.price
            }
          }),{overUnit:exceedPrice.price}],countSettle:"",countChe:"",countDun:"",countDai:""}],unit,valuationType:6,settlementType:settlementType
    }
  }else if(valuationType===3&&calculateType===2){

    $price = {
      price:[{countTime:valuationPrice,ladderObj:[{count:"",unit:""}],countSettle:calculatePricePiece,countDai:calculatePriceBag,countChe:calculatePriceCar,countDun:calculatePriceTon}],unit:"",valuationType:7,settlementType:settlementType
    }
  }else if(valuationType===4){
   $price = {
          price:[{sex:3,countFixed:dayPrice}],unit:"",valuationType:8,settlementType:settlementType
        }
   ...
   const $params = {
    ...
    if(!this.edit){
      const {outClassListLen,outaskName} = this.$store.state;
      this.form.classifyName = outaskName+'('+(outClassListLen+1)+')';
       let $userInfo = JSON.parse(localStorage.getItem('userInfo'));
      this.form.fullName = $userInfo.customerName;
    }
  ...
  this.$refs['form'].clearValidate();
    ...
    handleEditUnitPrice(){
        let res = JSON.parse( this.$store.state.outThusTaskDetail.price );
