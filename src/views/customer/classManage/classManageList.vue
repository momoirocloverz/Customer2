<template>
    <div class="gClassManagerList">
        ...
        <div class="table-box back-white" style="padding-top:20px;">
            <el-button type="primary" @click="handleAddClassTask" v-if="showAdd" :loading="verifyLoading"
                       style="margin-left:20px;margin-bottom:20px;">添加班务
            </el-button>
                ...
                <el-table-column label="操作" align="center" width="500">
                    <template v-slot="scope">
                        <el-button type="text" class="btn-icon btn-icon-view" size="mini"
                                   @click="handleToDetail(scope.row)">班务详情
                        </el-button>
                        <el-button type="text" v-if="scope.row.status===1||scope.row.status===2"
                                   class="btn-icon btn-icon-edit" size="mini" @click="handleEdit(scope.row)">编辑班务
                        </el-button>
                        <el-button type="text" v-if="scope.row.status===2" class="btn-icon btn-icon-reject" size="mini"
                                   :loading="verifyStopLoading" @click="handleStopClass(scope.row)">结束班务
                        </el-button>
                        <el-button type="text" v-if="scope.row.status===1" class="btn-icon btn-icon-view" size="mini"
                                   @click="handleDelete(scope.row)">删除班务
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
     ...
     <el-dialog title="添加班务成功" :visible.sync="showSuccessAdd" width="30%" center :before-close="handleClose"  >
        ...
        <el-button class="fixStyle1" @click="handleToClassList">继续为此任务添加班务</el-button>
          <el-button type="primary" @click="shutNow">返回任务</el-button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            // this.$api.getAssignmentInfoClassTaskList({assignmentInfoId:Number(assignmentInfoId)}).then(res=>{
        ...
        methods: {
            ...
            handleGetClassTaskDetail(classTaskId) {
                this.$api.getClassTaskDetail({classTaskId}).then(res => {
                    ...
                    this.$store.commit('SET_CLASS_FORM_STATE', res.data);
            ...
            handleGetClassList(type) {
                ...
                this.loading = true;
                this.$api.getAssignmentInfoClassTaskList({
                    pageNum,
                    pageSize,
                    assignmentInfoId: Number(assignmentInfoId)
                }).then(res => {
