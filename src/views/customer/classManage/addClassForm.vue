<template>
    <div class="add-class-form ">
        <el-dialog :title="edit?'编辑班务':'添加班务'" width="800px" :visible.sync="showAddClassForm" center
                ...
                <el-form-item label="班务经理:" prop="classManager" class="addRequireFixBug" v-show="form.isGrab == '1'" >
                    <div v-if="!edit">
                        <el-select v-model="form.classManager" placeholder="请选择" @change="handleChangeClassManager">
                            <el-option v-for="(it,i) in managerList" :label="it.managerNameAlias" :key="i"
            ...
            step: '00:30',
             ...
             start: '00:00',
              step: '00:30',
                ...
                <div>
                    <el-form-item label="已选中的工种:" v-if="form.priceLimit===1" class="addRequireFixBug" prop="alreadySelectWork">
                        <el-table
                                :data="form.workList"
                                border
                                style="width: 100%">
...
<script>
    ...
    export default {
        ...
        beforeDestroy() {
            //  this.$store.commit('CLEAR_CLASS_FORM_STATE');
            // this.$store.commit('CLEAR_THUS_TASK_DETAIL');
        },
        watch: {
            showAddClassForm: function (val, old) {
                if (val) {
                    if (this.$store.state.classFormState) {
                        this.handleSetEditData(this.$store.state.classFormState);
                    }
                    this.handleInitForm();
                } else {
                    this.$store.commit('CLEAR_CLASS_FORM_STATE');
        ...
        methods: {
            ...
            handleInitForm() {
                if (!this.edit) {
                    const {classListLen, taskName, thusTaskDetail} = this.$store.state;
                    const {latitude, longitude, workingPlace, authorizedNumber, cusAuthorizedNumber, startTime, endTime, workStartTime, endWorkType, workEndTime, price, industryList,deposit,isGrab} = thusTaskDetail;
