<template>
    <el-dialog class="addTaskDialogCon" :title="title" :visible.sync="addTaskVisible" width="740px" :center="true"
        ...
        <el-form label-width="200px" label-position="right" :model="form" :rules="rules" ref="form">
            <el-form-item label="任务名称：" prop="taskName">
                <el-input placeholder="不超过10个字" :disabled="isEdit" size="medium" v-model="form.taskName"
                          @keyup.native="handleSlice" maxlength="10"></el-input>
            ...
            <el-form-item label="指派给其他经理人：" required prop="selectManager" v-if="form.taskType==='3'">
                <div class="selectManagerSubCon">
                    <el-select class="controlSelect" v-model="form.selectManager" filterable placeholder="请选择"
                               :disabled="isEdit">
                        <el-option v-for="(item,index) in PartnerBindingList" :key="index" :label="item.partnerName"
                                   :value="item.partnerId"></el-option>
                    </el-select>
                    <div v-if="!PartnerBindingList.length" class="bindSelectInfoText">您还未绑定经理人，无法指定&nbsp;&nbsp;&nbsp;
                        <span class="bindText" @click="handleToBinding">去绑定</span></div>
                </div>
            </el-form-item>

            <el-form-item label="任务所属：" class="belongToItem" prop="belongTo"
                          v-show="(form.taskType=='1') || (form.taskType=='4')">
                <el-select :disabled="isEdit" v-model="form.belongTo" filterable placeholder="请选择"
                           @change="belongToChange(form.belongTo)">
                    <el-option v-for="(item,index) in subCustomerList" :key="index" :label="item.belongToName"
                               :value="`${item.belongTo},${item.belongToName}`"></el-option>
                </el-select>
            </el-form-item>
            ...
            <el-form-item label="开始时间：" prop="startTimeProp">
                <div class="startTimeStyleCon commonFlex">
                    <el-date-picker v-model="form.startTime0" :disabled="isEdit" type="date" placeholder="选择日期"
                                    value-format="yyyy-MM-dd" format="yyyy-MM-dd"
                                    :picker-options="pickerOptions0"></el-date-picker>
                    <el-time-select v-model="form.startTime1" :disabled="isEdit" placeholder="选择时间" value-format="HH:mm"
                                    format="HH:mm"
                                    :picker-options="{minTime: endStart,start: '00:00',step: '00:30',end: '23:30'}"></el-time-select>
                </div>
            </el-form-item>
            <el-form-item label="结束时间：" prop="endTimeProp" :required="form.settlementType == 3">
                <div class="startTimeStyleCon commonFlex">
                    <el-date-picker type="date" v-model="endTime0" placeholder="选择日期" value-format="yyyy-MM-dd"
                                    :picker-options="pickerOptions1"
                                    @change="endTimeChange('form','endTime0')"></el-date-picker>
                    <el-time-select placeholder="选择时间" v-model="endTime1" value-format="HH:mm"
                                    :picker-options="{start: '00:00',step: '00:30',end: '23:30'}"
                                    @change="endTimeChange('form','endTime1')"></el-time-select>
                </div>
            </el-form-item>
            ...
            <el-form-item label="计价方式：" class="valuationTypeItem" required>
                <!--计时，计量，计时+计量-->
                <el-select v-model="form.valuationType" placeholder="请选择" :disabled="isEdit"
                           @change="resetValuationType('form','valuationType')">
            ...
            <div v-if="!isEdit&&isSingleMetering">
                <el-form-item label="计量单价：">
                    <el-radio-group v-model="form.countType" @change="countTypeChange('form','countType')"
                                    :disabled="isEdit">
                ...
                <el-form-item v-if="form.countType == 1" prop="settleCount">
                    <el-input size="medium" v-model="form.settleCount" :disabled="isEdit" maxlength="10"
                              class="innerSpeSet"></el-input>
                    元/{{varUnit}}<br/>
                </el-form-item>
                <!--阶梯计价-->
                    ...
                    <div v-for="(item, index) in form.ladderObj" :key="index">
                        <el-row>
                            <el-col :span="12" :offset="4">
                                <img class="delIconImg" src="../../../../assets/images/delete_ladder.png" alt="删除"
                                     v-if="index != 0" @click="deleteLadder(index)">
                        ...
                        <el-row class="ladderFourItemCon">
                            <div class="divideWidth">
                                <el-form-item :prop="`ladderObj.${index}.count`" :rules="ladderObjRules.count">
                                    <el-input :disabled="isEdit" size="medium" v-model.trim.number="item.count"
                                              maxlength="10" class="short-input"></el-input>
                                </el-form-item>
                            ...
                            <div class="divideWidth removeSecondEffect">
                                <el-form-item :prop="`ladderObj.${index}.unit` " :rules="ladderObjRules.unit">
                                    <el-input :disabled="isEdit" size="medium" v-model.trim="item.unit" maxlength="10"
                ...
                <div v-if="form.countType == 2">
                    <el-row>
                        <el-col :span="8" :offset="4">
                            <el-button round size="medium" class="addBtn" @click="addLadder" v-if="!isEdit">添加阶梯
                            </el-button>
                        </el-col>
                    </el-row>
                </div>
                ...
                <el-form-item v-if="form.countType == 2 && form.ladderObj" prop="overUnitProp">
                    ...
                    <el-input :disabled="isEdit" size="medium" class="innerSingleStyle" v-model="form.overUnit"
                              maxlength="10"></el-input>
                    <span>元/{{varUnit}}</span>
                    <div>{{form.ladderObj[0].count || ''}}{{varUnit}}以下（含），
                        {{form.ladderObj[0].unit || ''}}元/{{varUnit}}
                    </div>
                    <div v-for="(item, index) in form.ladderObj" :key="index">
                        <div v-if="index != 0">{{Number((form.ladderObj[index-1].count)) +
                            1}}至{{item.count}}{{varUnit}}（含），{{item.unit}}元/{{varUnit}}
                        </div>
                    </div>
                    <div>{{form.ladderObj[form.ladderObj.length-1].count || ''}}{{varUnit}}以上，{{form.overUnit ||
                        ''}}元/{{varUnit}}
                    </div>
                </el-form-item>
            ...
            <div v-show="!isEdit&&isCombMetering">
                <el-form-item prop="countjian">
                    <el-input size="medium" v-model.trim="form.countjian" class="fourTextInput"
                              maxlength="10"></el-input>
                    元/件
                </el-form-item>
                <el-form-item prop="countDai">
                    <el-input size="medium" v-model.trim="form.countDai" class="fourTextInput"
                              maxlength="10"></el-input>
                    元/袋
                </el-form-item>
                <el-form-item prop="countChe">
                    <el-input size="medium" v-model.trim="form.countChe" class="fourTextInput"
                              maxlength="10"></el-input>
                    元/车
                </el-form-item>
                <el-form-item prop="countDun">
                    <el-input size="medium" v-model.trim="form.countDun" class="fourTextInput"
                              maxlength="10"></el-input>
                    元/吨
                </el-form-item>
            </div>
            <el-form-item v-if="form.taskType == 2 || form.taskType == 3" label="预估总量：" prop="estimateNumber">
                <el-input @input="renderUpdate()" size="medium" v-model="form.estimateNumber" maxlength="20"></el-input>
            </el-form-item>
            <el-form-item v-if="form.taskType == 2 || form.taskType == 3" label="预估总价：" prop="estimatePrice" required>
                <el-input @input="renderUpdate()" size="medium" v-model="form.estimatePrice" maxlength="10"></el-input>
                元
            </el-form-item>

            <el-form-item label="上班时间：" prop="workStartTime">
                <el-time-select class="workStartTimeStyleCon" v-model="form.workStartTime" placeholder="选择时间"
                                value-format="HH:mm" format="HH:mm"
                                :picker-options="{minTime: range,start: '00:00',step: '00:30',end: '23:30'}"></el-time-select>
            </el-form-item>
            <el-form-item label="下班时间：" prop="workEndTime">
                <div class="workEndTimeStyleCon commonFlex">
                    ...
                    <el-time-select v-model="form.workEndTime" placeholder="选择时间" value-format="HH:mm" format="HH:mm"
                                    :picker-options="{minTime: range,start: '00:00',step: '00:30',end: '23:30'}"></el-time-select>
                </div>
            </el-form-item>
            <el-form-item label="打卡地点：" prop="workingPlace">
                <el-tooltip :disabled="!form.workingPlace.length" effect="dark" :content="form.workingPlace"
                            placement="top-start">
                    <el-input placeholder="搜索地点，并进行选择" size="medium" v-model="form.workingPlace" @focus="viewMap"
                              v-if="!mapDia"></el-input>
            ...
            <div v-if="mapDia" class="mapContain">
                <baidu-map class="map-container" :scroll-wheel-zoom="true" :center="mapConfig.center"
                           :zoom="mapConfig.zoom">
                    <bm-local-search :keyword="mapConfig.keyword" :auto-viewport="true" :location="mapConfig.location"
            ...
            <el-form-item label="工种要求：" prop="inDicName">
                <el-checkbox-group v-model="industry" @change="handleCheckboxChange">
                    <el-checkbox v-for="(item,index) in taskIndustryNameList" :key="index"
                                 :disabled="initIndustry.includes(item.dicName)" :label="item.dicName"
                                 :value="item.dicName"></el-checkbox>
                </el-checkbox-group>
            </el-form-item>
            <el-form-item label="其他要求：">
                <el-input class="textStyle" type="textarea" :rows="2" placeholder="比如：以上零工必须要有投递经验"
                          v-model="form.otherDemand"></el-input>
            </el-form-item>
        </el-form>
...
<script>
    ...
    import rules from '../components/validRules.js';
    import initData from '../components/initData.js';
...
//                                  callback(new Error("请填写正确的单价"));
            ...
            let checkNum = (rule, value, callback) => {
                ...
                if (value) {
                    // console.log(value)
                    // if(Number(value) <= 0){
                    //     callback(new Error('预估总量必须大于0'));
                    // }
                } else {
                    callback(new Error('预估总量不能为空'));
                }
                callback();
            };
            let checkPrice = (rule, value, callback) => {
                if (value) {
                    console.log(value)
                    if(Number(value) <= 0){
                        callback(new Error('预估总价必须大于0'));
                    }
                } else {
                    callback(new Error('预估总价不能为空'));
                }
            };
...
//      },
                ...
                if (this.addOrEditStatus == 2) {
                    ...
                    this.$api.assignmentInfoDetail(params).then(res => {
                        ...
                        if (respCode === 0) {
                            ...
                            this.form.estimatePrice = data.expectMoney;
                            this.form.estimateNumber = data.expectPieceSize;
            ...
            saveAction() {
                ...
                let pickArr = [
                    ...
                    {name: 'estimatePrice', transNumber: true},
                ...
                let newParam = Object.assign(forNewStage, priceParam,{
                    expectPieceSize:this.form.estimateNumber,
                    expectMoney:this.form.estimatePrice,
                    cusAuthorizedNumber:this.form.authorizedNumber,
                    authorizedNumber:this.form.authorizedNumber,
                })
