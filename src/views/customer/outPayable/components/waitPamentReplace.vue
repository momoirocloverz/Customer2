<template>
    <div class="waitPamentReplaceContainer" v-loading="loading" :element-loading-text="loadingText">
        ...
        <div class="f14 pl30 pb15 pt8" v-if="$route.query.outsourcingCusName == '薪福多' && isXfdOrder">该收入批次有薪福多订单待付款，请先处理该订单。 <span @click="toWaitXfd()" class="co-orange">[点击查看]</span></div>
        <div class="f14 pl30 pb15 pt8" v-if="isHtOrder">该收入批次有订单待付款，请先处理该订单。 <span @click="toWait()" class="co-orange">[点击查看]</span>
        </div>
        <div class="table-box back-white">
            <div class="exportContainer">
                <div class="exportLeftCon">
                    <!--<el-button class="myplain-btn " size="small" :disabled="isDisabled" @click="onLine">线上付款</el-button>-->
                    <el-button class="myplain-btn " v-if="$route.query.outsourcingCusName != '薪福多'" size="small" :disabled="isDisabled" @click="xfdPay(1)">线上付款
                    </el-button>
                    <el-button class="myplain-btn " v-if="$route.query.outsourcingCusName == '薪福多'" size="small" :disabled="isDisabled" @click="xfdPay(2)">线上付款
                    </el-button>
            ...
            <el-table class="table-outline" :data="tableData" @selection-change="handleSelectionChange"
                ...
                <el-table-column label="姓名" align="left">
                    <!--和支付宝-->
                    <template v-slot="scope">
                        <el-tooltip class="item" effect="dark" content="未绑定银行卡和支付宝" placement="top-start"
                                    v-if="!scope.row.ifHasCard">
                            <img width="18" height="18"
                                 style="display: inline-block;margin-right:6px;vertical-align: middle;cursor: pointer"
                                 src="@/assets/icons/ts.png" alt="">
                        </el-tooltip>
        ...
        <el-dialog :title="`导入扣款项(${fromParams.outsourcingCusName} ${fromParams.batchNumber})`"
        ...
        <!--等待抽象为可复用组件-->
        <el-dialog :title="`导入扣款项(${fromParams.outsourcingCusName} ${fromParams.batchNumber})`"
                   :visible.sync="importVisible" class="centerDialog"
                   :before-close="emptyData">
                    ...
                    <el-form-item label="选择导入的文件：" class="importFormResetMargin">
                        <div class="importBtnCon import-btn-wrap">
                            ...
                            <el-upload
                                    ...
                                    :file-list="fileList"
                                    :auto-upload="false"
                                    :data={batchNumber:batchNumber,payType:2,showOutsourcing:true,outsourcingCusId:$route.query.outsourcingCusId}
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.queryWaitOrder();
            if(this.$route.query.outsourcingCusName == '薪福多'){
                this.queryXfdWaitOrder();
            }
            this.accountDetail()
            this.paytime()
            this.detail()
        ...
        methods: {
            ...mapMutations(['XFD_PARAMS','HT_PARAMS']),
            reset() {
            ...
            exportActionTwo() { //导出扣款项
                ...
                _this.loading = true
                _this.loadingText = '正在导出excel表...'
                let url = this.$importBaseURL + `salary/customer/settlement/adjust/export?userName=${param.userName}&payType=${param.payType}&batchNumber=${param.batchNumber}&settlementType=${param.settlementType}&payStatus=${param.payStatus}&customerId=${param.customerId}&outsourcingCusId=${param.outsourcingCusId}&showOutsourcing=${param.showOutsourcing}`;
               window.open(url)
                setTimeout(() => {
                    _this.loading = false
                }, 1500)
            ...
            goToHandlerDetails(row) {
                let query = Object.assign(row, {payStatus: '1', outsourcingCusName: this.fromParams.outsourcingCusName,outsourcingCusId:this.fromParams.outsourcingCusId})
                const {href} = this.$router.resolve({
