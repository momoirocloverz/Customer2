...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeService(){
                let _this = this;
                this.$api.xfdPerQuery(this.htParams).then(res => {
                    if (res.data.respCode == 0) {
                        ...
                        if (oldOrder == 1) { // 是否有订单（1是  其他否
                            this.$confirm(`您有订单已生成待付款, 请先处理该订单。`, `提示`, {
                            ...
                            this.fromParams = res.data.data;
                            this.tableData = this.fromParams.htPreGenerateVOS || [];
                        }
                    }
                }).catch(err => {
            ...
            refresh() {
                ...
                _this.request = true;
                _this.htParams.ifTalentPay = _this.radio;
                _this.$api.generateOrder(_this.htParams).then(res => {
                    if (res.data.respCode == 0) {
                        ...
                        if (oldOrder == 1) { // 是否有订单（1是  其他否
                            ...
                            _this.HT_PARAMS(_this.htParams)
                            _this.$router.push({
                                path: 'outXfdOrder',
                                query: {
                                    htMoney,
