<template>
    <div>
        <div class="overflow-box" style="width:646px;">
            <div class="box-title"><span>利润调整</span></div>
            <div class="box-body1">
                <div class="box-body-item">
                    <div class="box-body-form">
                        <el-form ref="form" :model="form" label-width="150px" label-position="left" :rules="rules">
                            ...
                            <el-form-item :label="item.adjustmentName"
                                          v-for="(item,index) in form.unsettleList"
                                          :key="index"
                                          :prop="'unsettleList.' + index + '.amount'"
                                          :rules="[
//                                              { required: true, message: '请输入邮箱地址', trigger: 'blur' },
                                               {pattern: /^(([0-9]\d*)(\.\d{1,2})?|0\.[0-9][1-9])$/, message: '请输入正确的金额', trigger: ['change']}
    ]"
                            >
                                <span v-if="item.adjustmentVal>0">增加 &nbsp;</span>
                                <span v-if="item.adjustmentVal<0">扣减 &nbsp;</span>
...
//          salaryId:this.$route.params.id,
        ...
        this.$api.profitadjustList(params).then((response) => {
      ...
      handleSubmit() {
        ...
        if(!valid){
          this.$message.error('至少填写一个调整项的调整金额')
          return false
        }
        const params ={
          taskManagerProfitId:this.taskManagerProfitId,
          taskProfitAdjustmentList:this.form.unsettleList
        }
        this.$api
            .profitadjustAdd(params)
            .then(response => {
