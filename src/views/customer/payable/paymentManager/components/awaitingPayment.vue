<template>
    <div class="marketPaymentAwaitingCon" v-loading="loading" :element-loading-text="loadingText">
        ...
        <!--<div class="f14 pl30 pb15 pt20" v-if="isXfdOrder">该收入批次有薪福多订单待付款，请先处理该订单。 <span @click="toWait()" class="co-orange">[点击查看]</span></div>-->
        <!--<div class="secondLine">-->
            <!--<div class="leftPart">-->
                <!--<el-button class="myplain-btn " size="small" :disabled="isDisabled" @click="offLinetrigger">线下付款</el-button>-->
                <!--<el-button class="myplain-btn " size="small" :disabled="isDisabled" @click="xfdPay">薪福多付款</el-button>-->
        ...
        <el-dialog :visible.sync="offlineShow" width="450px" class="base-dialog ">
            <div class="dialog-head">
                <p class="base-dialog-title">确认已进行线下付款？</p>
            </div>
            <span slot="footer" class="dialog-footer">
              <el-button size="middle" @click="offlineShow = false">取 消</el-button>
              <el-button size="small"  @click="taskConfirms">确 认</el-button>
          </span>
        </el-dialog>
                    ...
                    if (isNan) {
                        ...
                        if (isInteger) {
                            ...
                            if (numberFormat <= 0) {
                                ...
                                let floatLength = String(this.oneForm.amount).split('.')[1].length;
                                if (floatLength >= 2) {
//                                    let process = (Math.round(+numberFormat + 'e' + 2) / Math.pow(10, 2)).toFixed(2);                                    
                                    ...
                                    if( String(this.oneForm.amount).split('.')[0].length>8 ){
                                        callback(new Error("请填写短于8位的金额数"));
                                    }else{
                                        callback();
                                    }
                ...
                this.$api.schedulingSalarylist(reqParams).then(res=>{
                    ...
                    if( respCode == 0 ){
                        if(data && data.list){
                            ...
                            this.noSelectList = this.tableData.map((i) => i.greyStatus); //判断是否全部都是不可付
