        ...
        <div class="table-box back-white">
            <div class="exportContainer pt20 pb10">
                <div class="exportLeftCon">
                    <span class="pr30">
                        薪福多账户余额
                        <span class="co-orange-new">{{fromParams.xfdMoney | toFixeFortter2}}</span>元</span>
        ...
        >
            ...
            >
                ...
                <el-form-item
                        ...
                        style="margin-bottom:20px !important"
                >
                    <span>￥{{fromParams.xfdMoney | toFixeFortter2}} 元</span>
                </el-form-item>
                <el-form-item
                        label="支付密码："
                        prop="passwd"
        ...
        >
            ...
            >
                <el-form-item
                        ...
                        style="margin-bottom:20px !important"
                >
                    <span class="pr20">{{fromParams.financePhone}}</span>
                    <el-button
                            type="primary"
                            size="small"
                            class="newBtnStyle"
                            @click="getCode"
                            :disabled="xfdisMobile"
                    >{{xfdcodeTxt}}
                    </el-button>
                </el-form-item>
                <el-form-item
                ...
                >
                    <el-input
                            style="width:330px"
                            v-model="formPayXfd.verificationCode"
                            placeholder="请输入验证码"
                    ></el-input>
                </el-form-item>
                <el-form-item
                        ...
                        style="margin-bottom:20px !important"
                >
                    <span>请输入财务手机号收到的验证码</span>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
            <el-button @click="dialogVisibleTwo=false">取 消</el-button>
            <el-button type="primary" @click="taskConfirms">确 认</el-button>
      </span>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        mounted() {
            ...
            if(!this.$route.query.isWait){
                this.tableData = JSON.parse(this.fromParams.xfdPreQueryDetailVOS) || [];
                this.orderInfo = JSON.parse(this.fromParams.xfdPreQueryVO) || {};
            }
            this.accountDetail()
            this.refresh()
        },
        methods: {
            ...
            cancel(){ //取消订单
                    ...
                    this.$api.xfdCancal(params).then(res=>{
                        if(res.data.respCode == 0){
                            this.$alert('该订单状态已取消!', '提示', {
                                confirmButtonText: '确定',
                                callback: action => {
                                    if(this.$route.query.isOut){
                ...
                if(this.$route.query.isWait){ //如果代付查看订单的进来的话
                    _this.$api.getCurrentTime().then(res => {
                        if (res.respCode == 0) {
                            ...
                            _this.$api.xfdWaitingOrder({}).then(res => {
                                if (res.respCode == 0) {
                                    let {accountStatus, oldOrder, xfdPreQueryDetailVOS, xfdMoney, xfdPreQueryVO,financePhone} = res.data;
                                    _this.fromParams = { xfdMoney,financePhone,xfdPreQueryDetailVOS,xfdPreQueryVO};
                                    _this.tableData = _this.fromParams.xfdPreQueryDetailVOS;
                                    _this.orderInfo = _this.fromParams.xfdPreQueryVO;
                                    _this.timeOut()
                                }
                            }).catch(err=>{
                                console.log(err);
                    ...
                    _this.$api.getCurrentTime().then(res => {
                        if (res.respCode == 0) {
                            _this.currentTime = res.data  // 时间戳
                            _this.$api.generateOrder(_this.xfdParams).then(res => {
                                if (res.data.respCode == 0) {
                                    let {accountStatus, oldOrder, xfdPreQueryDetailVOS, xfdMoney, xfdPreQueryVO,financePhone} = res.data.data;
                                    _this.fromParams = { xfdMoney,financePhone,xfdPreQueryDetailVOS,xfdPreQueryVO};
                                    _this.tableData = _this.fromParams.xfdPreQueryDetailVOS;
                                    _this.orderInfo = _this.fromParams.xfdPreQueryVO;
                                    _this.timeOut()
            ...
            getTimexfd() {
                this.xfdcodeTime = this.xfdcodeTime - 1;
                this.xfdcodeTxt = this.xfdcodeTime + "秒后重新发送";
                if (this.xfdcodeTime == 0) {
                    this.xfdcodeTxt = "重新发送";
                    this.xfdcodeTime = 60;
                    this.xfdisMobile = false;
                    return;
                } else {
                    this.xfdisMobile = true;
                }
                setTimeout(this.getTimexfd, 1000);
            },
            getTime() {
            ...
            getCode() {
                let params = {
                    financePhone: this.fromParams.financePhone,
                    orderNo:this.orderInfo.orderNo
                };
                this.$api
                    .smsCode(params)
            ...
            payInline() {
                ...
                if (this.fromParams.xfdMoney < Number(this.orderInfo.totalRealFee)) {
                    return this.$message.error("薪福多余额不足，请充值！");
                }
                this.$api.xfdPayPass({password:this.formPay.passwd}).then(res=>{
                    if(res.data.respCode === 0){
                        _this.taskConfirms()
                    }else{
