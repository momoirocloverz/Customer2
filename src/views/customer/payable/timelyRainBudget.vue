...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.tableData = JSON.parse(this.fromParams.preGenerateVOS) || [];
            console.log(this.tableData)
            this.accountDetail()
        },
        methods: {
            ...mapMutations(['RAIN_PARAMS']),
            ...
            refresh() {
                ...
                _this.request = true;
                _this.rainParams.ifTalentPay = _this.radio;
                _this.$api.timelyRainRenerateOrder(_this.rainParams).then(res => {
                    ...
                    if (res.data.respCode == 0) {
                        ...
                        if (oldOrder == 1) { // 是否有订单（1是  其他否
                            ...
                            _this.RAIN_PARAMS(_this.rainParams)
                            _this.$router.push({
                                path: 'timelyRainOrder',
                                query: {
                                    balance,
                                    preQueryDetailVOS: JSON.stringify(preQueryDetailVOS),
                                    preQueryVO: JSON.stringify(preQueryVO),
                                }
                            });
                        }
                    }
