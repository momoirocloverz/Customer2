        ...
        <div class="table-box back-white">
            ...
            <div class="pl30 pr25 pb10 f14">
                <div class="lh25 co-333">
                    可发放合计：{{fromParams.talentPay}}人，
                    付款手续费{{fromParams.allPayFee | toFixeFortter2}}元，
                    信息服务费{{fromParams.serviceFee | toFixeFortter2}}元；
                    实发金额{{fromParams.allShouldPay | toFixeFortter2}}元
                </div>
            </div>
    ...
    export default {
        ...
        watch:{
            'radio':function (val,oldVal) {
                if(val == '0'){
                    this.tableData = JSON.parse(this.fromParams.xfdPreGenerateVOS);
                    this.fromParams.allShouldPay = this.$route.query.allShouldPay
                }else{
                    this.fromParams.allShouldPay = Number(this.$route.query.allShouldPay) + Number(this.$route.query.allPayFee);
        ...
        mounted() {
            this.fromParams = JSON.parse(JSON.stringify(this.$route.query))
            this.tableData = JSON.parse(this.fromParams.xfdPreGenerateVOS) || [];
            console.log(this.tableData)
            this.accountDetail()
        },
        methods: {
            ...mapMutations(['XFD_PARAMS']),
            ...
            refresh() {
                ...
                _this.request = true;
                _this.xfdParams.ifTalentPay = _this.radio;
                _this.$api.generateOrder(_this.xfdParams).then(res => {
                    if (res.data.respCode == 0) {
                        ...
                        if (oldOrder == 1) { // 是否有订单（1是  其他否
                            ...
                            _this.XFD_PARAMS(_this.xfdParams)
                            _this.$router.push({
                                path: 'xfdOrder',
                                query: {
                                    xfdMoney,
                                    financePhone,
                                    xfdPreQueryDetailVOS: JSON.stringify(xfdPreQueryDetailVOS),
                                    xfdPreQueryVO: JSON.stringify(xfdPreQueryVO),
                                }
                            });
                        }
                    }
