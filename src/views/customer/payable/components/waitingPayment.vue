      ...
      <el-table class="table-outline" :data="tableData" emptyText="暂无相关明细" style="width: 100%;">
        ...
        <el-table-column label="操作" show-overflow-tooltip align="center" >
          <template v-slot="scope">
              <div class="goToWaiting" @click="goToWaiting(scope.row)"><img class="operateIcon" :src="operateIcon"><span>付款</span></div>
          </template>
        </el-table-column>
      </el-table>
...
export default {
        ...
        switch (val) {
          ...
          case '5':
            ...
            this.formDetail.endDate = this.Moment(Date.now()).endOf('month').format('YYYY-MM-DD')
                  console.log(this.Moment(Date.now()).endOf('month').format('YYYY-MM-DD'))
  ...
  methods: {
    ...
    generateInfo(){
      let _this = this;
      if(_this.requestSalary){return}
      _this.requestSalary= true;
      let params = {};
      this.$api.calculateSalary(params).then(res =>{
        if(res.respCode === 0){
          console.log(res)
          _this.$message({
            message:`已生成批次日收入${res.data.daily}条,周收入${res.data.weekly}条,月收入${res.data.monthly}条,`,
            type: 'success'
          });
          _this.getPayList();
          _this.getScheduingList();
        }
      }).catch(err => {
        console.log(err)
      }).finally(()=>{
        setTimeout(()=>{
          _this.requestSalary= false;
        },500)
      })
    }, 
    ...
    goToWaiting(item){
      console.log(item);
      const { href } = this.$router.resolve({
        path:'/payable/waitPamentReplace',
        query:item
      })
      window.open(href, '_blank')
    },  
