    ...
    <div class="xfd-collect">
        <el-form ref="formDetail"
            ...
            <div class="f14 pb10 pt15 co-666">
                ...
                <span>支出—实发收入：{{successInfo.totalRealFee}}元，个税{{successInfo.totalTax}}元；合计（应发）{{successInfo.totalMoney}}元</span>
            </div>
        </el-form>
    ...
    export default {
        ...
        methods: {
            getData() {
                ...
                this.$api.xfdDays(params).then(res => {
                    let {data, respCode} = res;
                    if (respCode === 0) {
                        if (data.xfdOrderSumDOS.list) {
                            this.tableData = data.xfdOrderSumDOS.list;
