<template>
  <div class="billDetailCon"  v-loading="loading" element-loading-text="正在导出excel表...">
    <el-form ref="formDetail" :model="formDetail" label-width="90px" labelPosition="left" style="background-color:#fff;padding-left:20px;">
        <div class="search-zone">
           <el-input class="searchMaster" clearable placeholder="输入交易对象姓名搜索" prefix-icon="el-icon-search"   v-model="formDetail.toUserName" @clear="searchNow"></el-input>
          ...
          <div class="combine-zone" v-if="isShowComb">
            ...
            <div class="cusCon">
                <el-form-item label="自定义：" class="formSearchMargin cusForm" v-if="formDetail.date == '4'">
                  ...
                  --
                  <el-date-picker class="margin_left10" size="mini" v-model="formDetail.endDate" type="date" value-format="yyyy-MM-dd" placeholder="选择结束时间" @change="endDateChange('formDetail','endDate')">
                  </el-date-picker>
            ...
            <el-form-item label="业务类型：" class="formSearchMargin">
              <el-radio-group class="radio-search wrap" v-model="formDetail.note" @change="noteChange(formDetail.note)">
                <el-radio label="">不限</el-radio>
            ...
            <el-form-item label="收支分类：" class="formSearchMargin">
              <el-radio-group class="radio-search" v-model="formDetail.type" @change="inOutTypeChange(formDetail.type)">
                <el-radio label="">不限</el-radio>
...
<!--              外包费用充值<span class="fontsizecolor">{{totalData.recharge || 0}}</span>元，-->
              合计<span class="fontsizecolor">{{totalData.recharge || 0}}</span>元</div>
        ...
        合计<span class="fontsizecolor">&nbsp;{{totalData.totalOut || 0}}&nbsp;</span>元</div>          
      </div>
      <div>
        <el-button class="myplain-btn exportBtn" size="small"  :disabled="isDisabled" @click="exportExcel">导出</el-button>
      </div>
    </div>
    <div class="table-box back-white">
      <el-table class="table-outline" :data="tableData" emptyText="暂无相关信息" style="width: 100%;">
        <el-table-column label="时间" prop="receiveTime"  :formatter="timeFormatter" show-overflow-tooltip></el-table-column>
        <el-table-column label="业务类型" prop="note" show-overflow-tooltip></el-table-column>
        <el-table-column label="交易对象" prop="toUserName" :formatter="existFormatter" show-overflow-tooltip></el-table-column>
        <el-table-column label="收入" prop="inAmount" :formatter="moneyFormat" show-overflow-tooltip></el-table-column>
        <el-table-column label="支出" prop="outAmount" :formatter="moneyFormat" show-overflow-tooltip></el-table-column>
        <el-table-column label="期末余额（元）" prop="balance"  :formatter="moneyFormat" show-overflow-tooltip></el-table-column>
          <el-table-column label="备注" prop="remark" :formatter="existFormatter" show-overflow-tooltip></el-table-column>
      </el-table>
