<template>
    <div class="xfd-collect">
        <el-form ref="formDetail"
            ...
            <div class="f14 pb10 pt15 co-666">
                合计:
                <!--<span>收入—充值：{{successInfo.totalMoney}}元，合计{{successInfo.totalMoney}}元；</span>-->
                <span>支出—实发收入：{{successInfo.totalRealFee || 0}}元，个税{{successInfo.totalTax || 0}}元；合计（应发）{{successInfo.totalMoney || 0}}元</span>
            </div>
        </el-form>
        <div class="table-box back-white">
            <el-table class="table-outline" :data="tableData" emptyText="暂无相关信息" style="width: 100%;">
                ...
                <!--<el-table-column label="收入">-->
                    <!--<el-table-column prop="totalMoney" label="充值" :formatter="moneyFormat"></el-table-column>-->
                    <!--<el-table-column prop="totalMoney" label="合计" :formatter="moneyFormat"></el-table-column>-->
                <!--</el-table-column>-->
                <el-table-column label="支出">
                    ...
                    <el-table-column prop="totalMoney" label="合计(应发收入)" :formatter="moneyFormat"></el-table-column>
                </el-table-column>
            </el-table>
    ...
    export default {
        ...
        methods: {
            getData() {
                ...
                this.$api.timelyRainDetailsDays(params).then(res => {
                    let {data, respCode} = res;
                    if (respCode === 0) {
                        if (data.commonOrderSumDOS.list) {
                            this.tableData = data.commonOrderSumDOS.list;
                            this.pageCount = data.commonOrderSumDOS.pages;
                        }else{
                            this.tableData = [];
                            this.pageCount = 1;
                        }
