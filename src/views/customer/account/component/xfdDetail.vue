<template>
    <div class="xfd-detail" v-loading="loading" element-loading-text="正在请求中...">
        <el-form ref="formDetail" :model="formDetail" label-width="100px" labelPosition="left" style="background-color:white;padding-left:20px;padding-bottom: 15px">
            ...
            <div class="f14 pb10 pt10 co-666">
                合计: 成功发放{{Number(successInfo.successCount).toFixed(0)}}人 ; {{Number(successInfo.successMoney).toFixed(2)}}元
                <span class="pl20">发放失败{{(Number(successInfo.totalCount) - Number(successInfo.successCount)).toFixed(0) || 0}}人 ; {{(Number(successInfo.totalMoney) - Number(successInfo.successMoney)).toFixed(2) || 0.00}}元</span>
            </div>
        </el-form>
        <div class="table-box back-white">
            <el-table class="table-outline" :data="tableData" emptyText="暂无相关信息" style="width: 100%;">
                ...
                <el-table-column label="订单发放人数" prop="successPeople" align="center" min-width=""></el-table-column>
                <el-table-column label="订单发放金额（元）" prop="successMoney" :formatter="_number" align="center" min-width="" ></el-table-column>
    ...
    export default {
        ...
        methods:{
            ...
            getList(){
                ...
                this.$api.xfdOrders(params).then(res=>{
                    if(res.respCode === 0){
                        this.xfdAccountTotal = res.data.xfdMoney || 0;
                        this.successInfo = res.data.xfdOrderSumDO || {};
                        ...
                        this.$emit('showTotal',{xfdAccountTotal:res.data.xfdMoney})
                    }
                    console.log(res)
                }).catch(err=>{
                    console.log(err)
