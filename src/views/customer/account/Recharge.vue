          ...
          <div>
            <el-form ref="form" :model="form" label-width="110px" :rules="rules">
              <el-form-item label="充值金额：" prop="idCard">
                ...
                <span class="charge-label">每笔充值收取{{charge}}元的手续费</span>
              </el-form-item>
            </el-form>
          </div>
        </div>
        <div class="box-footer">
          <el-button round size="medium" type="primary" @click="lastFeeDia = true">确定</el-button>
          <el-button round size="medium" @click="$router.go(-1)">取 消</el-button>
        </div>
      </div>
    </div>
    <el-dialog :visible.sync="lastFeeDia" width="600px" center>
      <div class="dialog-head">
        <span>您正在第三方支付页面充值</span>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button round size="medium" type="primary" @click="check">充值已完成</el-button>
        <el-button class="btnEdit" round size="medium" @click="lastFeeDia = false">修改充值信息</el-button>
      </span>
    </el-dialog>
  </div>
</template>
