<template>
    <div class="acc-xfd">
        <div class="bg-fff pt15 pb15 pl30 pr30">
            <span class="f14 co-333">开户状态: {{xfdStatus | forStatus}}</span>
            <el-button v-if="xfdStatus == '3' || xfdStatus == '5'" @click="routeToDetail()" class="co-orange-new" type="text">查看失败原因</el-button>
        </div>
        <div class="bg-fff mt15 pt30 pb20 pl30 pr30">
            <el-steps :active="2" align-center style="width: 50%;margin: auto">
        ...
        <div class="pt20 mt15 bg-fff pl30 pr30">
            <div class="text-xfd pt15 pl20 pr20">线上提交资料即可便捷的进行薪福多开户，成功开户后即可使用薪福多付款。资料提交需要审核，审核通过开户需x天。
...
<script>
    export default {
        ...
        methods: {
                ...
                this.$api.getXfdaccount({}).then(res => {
                    if (res.respCode === 0) {
                        ...
                        if(Number(res.data.status) <= 1){
                            this.$router.push({name:'accountXfd'})
                        }
                        localStorage.xfdStatus = res.data.status
                        this.xfdStatus = res.data.status
            ...
            routeToDetail() {
                this.$alert(`${this.userInfo.reason}`, '失败原因：', {
            ...
            save() {
                ...
                this.$api.accountUpdate(params).then(res => {
